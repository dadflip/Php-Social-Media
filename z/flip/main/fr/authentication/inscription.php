...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($_POST['username'], $_POST['firstname'], $_POST['lastname'], $_POST['email'], $_POST['password'], $_POST['confirm-password'])) {
            ...
            // Chiffrer les clés RSA avant de les stocker
            $encryptedPrivateKey = openssl_encrypt($rsaKeys['privateKey'], 'aes-256-cbc', $password, 0, substr(sha1($password), 0, 16));

            // Insérer l'utilisateur dans la base de données
            $stmtInsertUser = $databaseManager->getAdminConnection()->prepare("INSERT INTO users (user_id, username, email, password, rsa_public_key, rsa_private_key) VALUES (?, ?, ?, ?, ?, ?)");
            $stmtInsertUser->bind_param("isssss", $userId, $username, $email, $hashedPassword, $rsaKeys['publicKey'], $encryptedPrivateKey);
